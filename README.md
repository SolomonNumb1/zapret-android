...
Если сайт не открывается и его нет в списках — добавьте его в свой список через меню:

```bash
bash /data/local/tmp/zapret/service.sh
# пункт 9 «Мои домены» — посмотреть / добавить / удалить
```

Домены хранятся в `/data/local/tmp/zapret/lists/list-general-user.txt`, по одному на строку.
Запись матчит **точный** хост (SNI), поэтому `site.ru` не покроет `www.site.ru`. Чтобы
сайт и все поддомены разом — пишите `*.site.ru`. При добавлении дубликаты и уже имеющиеся
в базовом списке домены пропускаются.

Обновления (пункт 8 меню или `service.sh update_check`) **не трогают** файлы `*-user.txt`:
они не входят в состав репозитория, поэтому при обновлении заменяются только базовые списки,
стратегии и скрипты. Ваши домены переживают обновление и не дублируются, даже если домен
позже появится в базовом списке.

Такие же user-файлы генерируются автоматически для всех `*-user.txt`, на которые ссылаются
стратегии (сейчас это `list-general-user.txt`, `list-exclude-user.txt`, `ipset-exclude-user.txt`).
Править файлы напрямую можно только через `su`, поэтому проще пользоваться меню.
